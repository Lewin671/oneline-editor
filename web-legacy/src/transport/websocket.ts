...
import { MessageReader, MessageWriter, DataCallback, Disposable, Message, PartialMessageInfo } from 'vscode-jsonrpc';
...
class WebSocketMessageWriter implements MessageWriter {
  private errorEmitter: Array<(data: [Error, Message | undefined, number | undefined]) => void> = [];
  private closeEmitter: Array<() => void> = [];
  ...
  private async sendWithRetry(message: Message, attempt: number): Promise<void> {
    if (this.socket.readyState !== WebSocket.OPEN) {
      if (attempt < this.maxRetries) {
        ...
        this.errorEmitter.forEach(listener => listener([error, message, attempt]));
        throw error;
      }
    }
    ...
    try {
      ...
      if (attempt < this.maxRetries) {
        ...
        this.errorEmitter.forEach(listener => listener([error as Error, message, attempt]));
        throw error;
      }
    }
  }

  onError(listener: (data: [Error, Message | undefined, number | undefined]) => void): Disposable {
    this.errorEmitter.push(listener);
    return {
      dispose: () => {
        const index = this.errorEmitter.indexOf(listener);
